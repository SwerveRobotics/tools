...
namespace NativeUsbLib
    ...
    public abstract class Device
        ...
        /// <summary>
        /// The underlying USB device path in the case of a HID device
        /// </summary>
        protected string m_UsbDevicePath = string.Empty;
            ...
            if (handel.ToInt32() != UsbApi.INVALID_HANDLE_VALUE)
                ...
                if (UsbApi.DeviceIoControl(handel, UsbApi.IOCTL_USB_GET_NODE_CONNECTION_DRIVERKEY_NAME, ptrDriverKey, nBytes, ptrDriverKey, nBytes, out nBytesReturned, IntPtr.Zero))
                    ...
                    if (!m_IsHub)
                        ...
                        string devPath = GetDevicePathByKeyName(this.DriverKey);
                        if (devPath.Length > 0)
                            {
                            m_DevicePath = devPath; // Start with USB DevicePath
                            if (this.m_DeviceDescriptor != null)
                                {
                                // Replace USB DevicePath with HidDevicePath if VID, PID, and SerialNumber match
                                string tmp = GetHidDevicePath(m_DeviceDescriptor);
                                if (tmp.Length > 0)
                                    {
                                    ...
                                    m_UsbDevicePath = m_DevicePath; // Saved underlying USB device name
                                    m_DevicePath = tmp; // HID device name
                                    }
                                }
                            }
                        }
                            ...
                            if (UsbApi.SetupDiGetDeviceInterfaceDetail(handel, ref deviceInterfaceData,
                                ...
                                if (deviceInterfaceDetailData.DevicePath.Contains(strSearch))
                                    ...
                                    if (HidSerialNumberMatches(deviceInterfaceDetailData.DevicePath))
                                        {
                                        System.Diagnostics.Debug.WriteLine(string.Format("HidPath:{0}", deviceInterfaceDetailData.DevicePath));
            ...
            if (hnd.IsInvalid)
                ...
                try
                    ...
                    if (UsbApi.HidD_GetSerialNumberString(hnd, serialNumber, serialNumber.Capacity))
                        {
                        return serialNumber.ToString() == this.SerialNumber;
                        }
                    return false;
                    }
                finally
